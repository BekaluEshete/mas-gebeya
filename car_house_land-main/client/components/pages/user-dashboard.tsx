...
export function UserDashboard() {
  const { user, favorites, setIsAuthModalOpen, removeFromFavorites, cars, houses, lands, machines } = useApp()
  ...
  const postedItems = user?._id ? [
    ...
    ...(houses || []).filter((house: any) => String(house.sellerId) === String(user._id)).map((house: any) => ({ ...house, type: "house" })),
    ...(lands || []).filter((land: any) => String(land.sellerId) === String(user._id)).map((land: any) => ({ ...land, type: "land" })),
    ...(machines || []).filter((machine: any) => String(machine.sellerId) === String(user._id)).map((machine: any) => ({ ...machine, type: "machine" })),
  ] : []
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-8">
          ...
          <Card className="bg-white border-blue-100 shadow-lg hover:shadow-xl transition-all duration-300 animate-slide-in-up">
            <CardContent className="p-6 text-center">
              ...
              <div className="text-2xl font-bold text-gray-900">{postedItems?.length || 0}</div>
              <div className="text-gray-600 text-sm">Posted Items</div>
            </CardContent>
          </Card>
        </div>
        ...
        <div className="bg-white rounded-2xl shadow-lg border border-blue-100 animate-fade-in">
          <Tabs value={activeTab} onValueChange={setActiveTab} className="p-6">
            ...
            <TabsContent value="posted-items" className="space-y-6 mt-6">
              <Card className="border-gray-200">
                <CardHeader>
                  <CardTitle className="flex items-center space-x-2">
                    <Package className="w-5 h-5 text-blue-600" />
                    <span>Your Posted Items</span>
                  </CardTitle>
                </CardHeader>
                <CardContent>
                  {postedItems && postedItems.length > 0 ? (
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                        ...
                        return (
                          ...
                          >
                            <Link href={itemUrl} className="block">
                              ...
                              <div className="p-4">
                                <h3 className="font-semibold text-gray-900 capitalize mb-1">{item.type}</h3>
                                <p className="text-sm text-gray-900 font-medium mb-1 line-clamp-2">{itemTitle}</p>
                                <p className="text-sm text-blue-600 font-semibold mb-3">{itemPrice}</p>
                                <p className="text-xs text-gray-500">{item.city || ""}{item.region ? `, ${item.region}` : ""}</p>
                              </div>
                            </Link>
                          </div>
                        )
                    ...
                    <div className="text-center py-8">
                      ...
                      <p className="text-gray-600">No posted items yet</p>
                      <p className="text-sm text-gray-500 mb-4">Start posting your items to see them here</p>
                      <Link href="/post-items">
